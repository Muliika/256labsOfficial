...
function FloatingPaths({ position }: { position: number }) {
  ...
  useEffect(() => {
    ...
    styleSheet.textContent = paths
      .map(
        (path) => `
        @keyframes travelPath-${position}-${path.id} {
          0% {
          ...
          <g key={path.id}>
            ...
            <path
              ...
              style={{
                ...
                animation: `travelPath-${position}-${path.id} ${path.duration}s linear infinite`,
...
function FlippedFloatingPaths({ position }: { position: number }) {
  ...
  useEffect(() => {
    ...
    styleSheet.textContent = paths
      .map(
        (path) => `
        @keyframes travelPathFlipped-${position}-${path.id} {
          0% {
            stroke-dashoffset: ${path.dashLength + path.dashGap};
          }
          100% {
          ...
          <g key={`flipped-${path.id}`}>
            ...
            <path
              ...
              style={{
                strokeDasharray: `${path.dashLength} ${path.dashGap}`,
                opacity: 0.6 + path.id * 0.02,
                animation: `travelPathFlipped-${position}-${path.id} ${path.duration}s linear infinite`,
                animationDelay: `${path.delay}s`,
              }}
            />
          </g>
